...
Add one API key to `.env`. See `.env.example` for the full list of keys. You need a key only to run the pipeline against a real repo (`python -m workflow path/to/repo`). The test suite needs no key. The test suite needs no network access.
...
- Parse LLM YAML output through `coderay_utils.yaml_call`. This function retries with a varied prompt when it gets a bad response. Do not add a new parse-and-retry loop in `workflow/nodes.py`.
- Escape untrusted input before it reaches HTML or Mermaid output. Untrusted input includes the target repo's own files. It also includes anything the LLM echoes back from those files. `workflow/__main__.py` once had a confirmed stored-XSS (cross-site scripting) bug here. Read `.full-review/02a-security.md` before you change that file's rendering code.
...
- Add a new output lens by adding one file to `workflow/instructions/`. This step needs no code change and no registration step.
