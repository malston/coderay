...
Add one API key to `.env`. See `.env.example` for the full list of keys. You need a key only to run the pipeline against a real repo (`crawl tour path/to/repo`). The test suite needs no key. The test suite needs no network access.
