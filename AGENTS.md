...
Crack runs five PocketFlow analyses: a multi-chapter tour, a server-side backend flow analysis, a multi-service architecture map, an API surface guide, a data-model tour, and a git-history story. See `CLAUDE.md` for build/test commands, architecture, and project conventions -- read it before making changes.
