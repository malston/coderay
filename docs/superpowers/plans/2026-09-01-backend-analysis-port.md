...
In `CLAUDE.md` and `AGENTS.md`, under "Architecture Overview", note that `crack` now dispatches to more than one analysis: `tour` (the five-node chapter pipeline, unchanged) and `backend` (`BuildBundle -> Pipeline -> LayerCode -> Trace -> OverviewNode`). Add to "Conventions & Patterns" that the four card-family analyses declare `SECTIONS` and `THEME` and are rendered by `crack/core/render.py`, and that an analysis with a page shape that does not fit declares `render_html`/`render_markdown` instead. Keep both files in sync; they are independent files, not symlinks.
