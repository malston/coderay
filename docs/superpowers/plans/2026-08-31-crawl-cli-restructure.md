# Crawl CLI Restructure Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Restructure coderay's single-pipeline `workflow`/`coderay_utils` layout into a `src/crawl/` package with a `crawl` console script that dispatches to named analysis subcommands (today: `crawl tour <repo>`), with no behavior change.

**Architecture:** Move `coderay_utils/*` to `src/crawl/core/*` (shared plumbing), move the pipeline (`workflow/nodes.py`, `flow.py`, `prompts/`, `instructions/`, `graph/`) to `src/crawl/analyses/tour/*`, and split `workflow/__main__.py` into a thin `cli.py` dispatcher plus `analyses/tour/render.py` (all of today's rendering/dry-run/session-summary logic). Each task is a pure move + import-path update, landing green before the next task starts.
...
- **No `crawl all`** this iteration (spec Non-goals).
...
```text
pyproject.toml                          # package "crawl", src layout, console script "crawl"
src/crawl/
  __init__.py
...
### Task 1: `pyproject.toml` skeleton and empty `src/crawl/` package

**Files:**

- Modify: `pyproject.toml`
- Create: `src/crawl/__init__.py`
- Create: `src/crawl/core/__init__.py` (empty placeholder, filled in Task 2)
- Create: `src/crawl/analyses/__init__.py` (empty placeholder, filled in Task 5)
- Create: `src/crawl/analyses/tour/__init__.py` (empty placeholder, filled in Task 5)
- Test: `tests/test_package_skeleton.py`

**Interfaces:**

- Produces: an installed, importable `crawl` package with no functional code yet. `workflow`/`coderay_utils` are untouched and still fully functional in this task.
...
Expected: FAIL with `ModuleNotFoundError: No module named 'crawl'`

- [ ] **Step 3: Create the empty package tree**

```bash
mkdir -p src/crawl/core src/crawl/analyses/tour
touch src/crawl/__init__.py src/crawl/core/__init__.py \
      src/crawl/analyses/__init__.py src/crawl/analyses/tour/__init__.py
```

- [ ] **Step 4: Update `pyproject.toml`** — switch to `src` layout and add `crawl` to the packages list alongside the still-present `workflow`/`coderay_utils` entries (both package trees coexist until Task 6). **Do not change `name` yet** — see the note below.
...
packages = [
    ...
    "crawl", "crawl.core", "crawl.analyses", "crawl.analyses.tour",
]
...
Note: `package-dir` maps the bare `""` root to `src/` (where `crawl` lives) while pinning `workflow` and `coderay_utils` back to the repo root, since those two packages aren't moving until later tasks. This dual mapping is temporary scaffolding removed in Task 6 once `workflow`/`coderay_utils` are deleted.

**`[project] name` stays `"coderay"` through Task 4.** `workflow/__main__.py`'s `--version` flag calls `importlib.metadata.version('coderay')` _eagerly_, at `argparse.add_argument()` time — every invocation of `main()`, not just `--version` itself, runs that lookup. If `name` flipped to `"crawl"` here, that lookup would raise `PackageNotFoundError` and break `workflow/__main__.py` (and every test that calls its `main()`, including the dry-run tests) for the four tasks before `workflow/__main__.py` is replaced. One `[project] name` covers every package this distribution builds — `crawl`, `workflow`, and `coderay_utils` all ship under it regardless of what the distribution itself is called — so nothing about building or importing `crawl.*` here needs the rename yet. The rename happens in Task 5 Step 9, in the same step that deletes `workflow/__main__.py` and its `version('coderay')` call.

- [ ] **Step 5: Regenerate the lockfile and sync**

Run: `uv lock && uv sync`
Expected: resolves cleanly; `uv.lock` still shows the single `coderay==0.1.0` (editable) distribution, now building both the old `workflow`/`coderay_utils` packages and the new empty `crawl` package tree.
...
```bash
git add pyproject.toml uv.lock src/crawl tests/test_package_skeleton.py
git commit -m "Add empty crawl package skeleton alongside workflow/coderay_utils"
```

---

### Task 2: Move `coderay_utils/*` → `src/crawl/core/*`
...
In `src/crawl/core/call_llm.py`, update the docstring and `CACHE_DIR`:
...
In `src/crawl/core/pricing.py`, update the docstring and `CONFIG_DIR`:
...
- [ ] **Step 3: Update `src/crawl/core/__init__.py`** to the same re-export shape as `coderay_utils/__init__.py`:
...
`tests/test_call_llm.py` line 392: `from crawl.core.call_llm import resolve_provider_and_model`
...
`tests/test_pricing.py` — every `from coderay_utils.pricing import ...` and `import coderay_utils.pricing as pricing_module` (lines 1, 34, 46, 58, 67, 75, 84-85, 96-97, 109, 117, 127, 134, 153, 168, 183, 194, 208, 218) becomes `from crawl.core.pricing import ...` / `import crawl.core.pricing as pricing_module` respectively — mechanical find-replace of `coderay_utils.pricing` → `crawl.core.pricing`.

`tests/test_prompts.py` line 4: `from crawl.core import fill`
`tests/test_prompts.py` line 23: `from crawl.core.call_llm import CACHE_BREAKPOINT`

`tests/conftest.py` line 6: `import crawl.core.pricing as pricing_module`
...
git add -A
git commit -m "Move coderay_utils to crawl.core; rename config/cache dirs to crawl"
```

---

### Task 3: Move `workflow/graph/languages/` → `src/crawl/analyses/tour/graph/languages/`

**Files:**

- Move: `workflow/graph/__init__.py` → `src/crawl/analyses/tour/graph/__init__.py`
- Move: `workflow/graph/languages/__init__.py` → `src/crawl/analyses/tour/graph/languages/__init__.py`
- Move: `workflow/graph/languages/python.py` → `src/crawl/analyses/tour/graph/languages/python.py`
- Move: `workflow/graph/languages/javascript.py` → `src/crawl/analyses/tour/graph/languages/javascript.py`
- Move: `workflow/graph/languages/typescript.py` → `src/crawl/analyses/tour/graph/languages/typescript.py`
- Modify: `workflow/nodes.py` (import line only)
- Modify: `tests/test_graph_python.py`, `tests/test_graph_javascript.py`, `tests/test_graph_typescript.py` (import paths only)

**Interfaces:**

- Consumes: `src/crawl/analyses/tour/` package from Task 1.
- Produces: `crawl.analyses.tour.graph.languages.REGISTRY` (dict of file extension → extractor module, same shape as today's `workflow.graph.languages.REGISTRY`), consumed by `workflow/nodes.py`'s `ExtractGraph` in this task, and later by the moved `nodes.py` in Task 4.

- [ ] **Step 1: Move the files**

```bash
mkdir -p src/crawl/analyses/tour/graph/languages
git mv workflow/graph/__init__.py src/crawl/analyses/tour/graph/__init__.py
git mv workflow/graph/languages/__init__.py src/crawl/analyses/tour/graph/languages/__init__.py
git mv workflow/graph/languages/python.py src/crawl/analyses/tour/graph/languages/python.py
git mv workflow/graph/languages/javascript.py src/crawl/analyses/tour/graph/languages/javascript.py
git mv workflow/graph/languages/typescript.py src/crawl/analyses/tour/graph/languages/typescript.py
...
# after
import crawl.analyses.tour.graph.languages.python as python_module
from crawl.analyses.tour.graph.languages.python import imports
```
...
packages = [
    "workflow",
    "crawl", "crawl.core", "crawl.analyses", "crawl.analyses.tour",
    "crawl.analyses.tour.graph", "crawl.analyses.tour.graph.languages",
]
```
...
git status --porcelain -uall  # confirm only expected files changed before sweeping them in
git add -A
git commit -m "Move import-graph extractors to crawl.analyses.tour.graph"
```

---

### Task 4: Move `workflow/nodes.py`, `flow.py`, `prompts/`, `instructions/` → `src/crawl/analyses/tour/`

**Files:**

- Move: `workflow/nodes.py` → `src/crawl/analyses/tour/nodes.py`
- Move: `workflow/flow.py` → `src/crawl/analyses/tour/flow.py`
- Move: `workflow/prompts/*.md` (4 files) → `src/crawl/analyses/tour/prompts/*.md`
- Move: `workflow/instructions/*.md` (4 files) → `src/crawl/analyses/tour/instructions/*.md`
- Modify: `src/crawl/analyses/tour/nodes.py` (docstring path references, `resources.files()` call, import lines)
- Modify: `src/crawl/analyses/tour/flow.py` (import line only)
- Modify: `workflow/__main__.py` (import lines only)
- Modify: `tests/test_nodes.py`, `tests/test_flow.py` (import paths only)
- Modify: `tests/test_main.py` line 23 — `from workflow.nodes import slug` must become `from crawl.analyses.tour.nodes import slug` in this task, not Task 5. This line is separate from the `from workflow.__main__ import (...)` block Task 5 handles; `workflow/nodes.py` stops existing once this task moves it, so leaving this line untouched would break `tests/test_main.py` collection between this task and Task 5.
- Modify: `tests/test_main.py` line 106 (comment only) — the regression-test comment says `workflow.nodes generates chapter filenames via slug()`; update the module reference to `crawl.analyses.tour.nodes` so the comment doesn't describe a path that no longer exists. Leave the `coderay-e06` bead-ID reference alone — that's a historical identifier, not a path.
- Modify: `src/crawl/core/llm.py` line 14 (docstring only) — `read_prompt`'s docstring says `` `workflow/prompts/` stays the source of truth for every prompt.`` Update to `` `crawl/analyses/tour/prompts/` stays the source of truth for every prompt.`` since this task is what moves that directory.

**Interfaces:**

- Consumes: `crawl.analyses.tour.graph.languages.REGISTRY` (Task 3), `crawl.core` (Task 2).
- Produces: `crawl.analyses.tour.nodes.{PipelineState, SmartCrawl, ExtractGraph, Analyze, Relate, WriteChapters, CODEBASE_BUDGET, PROMPTS_DIR, INSTRUCTIONS_DIR, load_instructions, slug}` and `crawl.analyses.tour.flow.create_tour_flow()` — consumed by `workflow/__main__.py` in this task, and by `analyses/tour/__init__.py` and `render.py` in Task 5.

- [ ] **Step 1: Move the files**

```bash
git mv workflow/nodes.py src/crawl/analyses/tour/nodes.py
git mv workflow/flow.py src/crawl/analyses/tour/flow.py
mkdir -p src/crawl/analyses/tour/prompts src/crawl/analyses/tour/instructions
git mv workflow/prompts/analyze-relationships.md src/crawl/analyses/tour/prompts/analyze-relationships.md
git mv workflow/prompts/identify-abstractions.md src/crawl/analyses/tour/prompts/identify-abstractions.md
git mv workflow/prompts/select-files.md src/crawl/analyses/tour/prompts/select-files.md
git mv workflow/prompts/write-chapter.md src/crawl/analyses/tour/prompts/write-chapter.md
git mv workflow/instructions/architecture-review.md src/crawl/analyses/tour/instructions/architecture-review.md
git mv workflow/instructions/beginner-tutorial.md src/crawl/analyses/tour/instructions/beginner-tutorial.md
git mv workflow/instructions/onboarding-guide.md src/crawl/analyses/tour/instructions/onboarding-guide.md
git mv workflow/instructions/security-audit.md src/crawl/analyses/tour/instructions/security-audit.md
rmdir workflow/prompts workflow/instructions 2>/dev/null || true
```

- [ ] **Step 2: Update `src/crawl/analyses/tour/nodes.py`**
...
Line 30 (already updated to `crawl.analyses.tour.graph.languages` in Task 3 — no change here).
...
# after
    """Parses each selected file with a per-extension tree-sitter extractor
    (crawl/analyses/tour/graph/languages/) and records import edges that land inside
```

- [ ] **Step 3: Update `src/crawl/analyses/tour/flow.py`** line 3:
...
**Note:** `tests/test_nodes.py` and `tests/test_flow.py` each import `coderay_utils.llm` too (as `llm_module`), on top of the `workflow.nodes`/`workflow.flow` imports below. That line was already updated to `crawl.core.llm` in Task 2 — `coderay_utils/` no longer exists by the time this task starts, so it couldn't still say `coderay_utils.llm` here. Only the `nodes_module`/`flow` lines below are this task's responsibility.

`tests/test_nodes.py` lines 3-5 (line 3, `import crawl.core.llm as llm_module`, is already correct from Task 2 — leave it):
...
`src/crawl/core/llm.py` line 14 (docstring only, in `read_prompt`):
...
[tool.setuptools.package-data]
"crawl.analyses.tour" = ["prompts/*.md", "instructions/*.md"]
```
...
git status --porcelain -uall  # confirm only expected files changed before sweeping them in
git add -A
git commit -m "Move pipeline nodes, flow, prompts, and instructions to crawl.analyses.tour"
```
...
- Create: `src/crawl/core/runner.py`
- Create: `src/crawl/cli.py`
- Create: `src/crawl/analyses/tour/render.py`
- Modify: `src/crawl/analyses/tour/__init__.py` (was an empty placeholder from Task 1)
- Modify: `src/crawl/analyses/__init__.py` (was an empty placeholder from Task 1)
- Modify: `tests/test_main.py` (imports + the `--version` subprocess test)
- Modify: `src/crawl/analyses/tour/nodes.py` (docstring text only — five `PipelineState` field docstrings name `workflow.__main__` as the reader of `selected_files`/`abstractions`/`order`/`relationships`/`chapters`; since this task is what replaces `workflow/__main__.py` with `render.py`, update each to say `crawl.analyses.tour.render` instead. Flagged as a Minor finding by Task 4's reviewer — fixing it here since this task is what makes the old reference stale.)
- Modify: `tests/test_nodes.py` line 373 (comment only) — `# (workflow/__main__.py:68); the rollup has no file set to check` names a line number in the file this task deletes; update the reference to `crawl/analyses/tour/render.py` (the line number is approximate in the original comment already — keep it approximate, don't chase an exact new line number).
- Delete: `workflow/__main__.py`

**Interfaces:**

- Consumes: `crawl.analyses.tour.nodes` and `crawl.analyses.tour.flow` (Task 4), `crawl.core` (Task 2).
- Produces:
  - `crawl.core.runner.run_flow(flow, shared, out_dir, dump_state) -> None` — runs `flow.run(shared)`; on any exception, calls `dump_state(shared, out_dir)`, prints `f"\nPipeline failed. Wrote partial run state to {state_path}"`, and re-raises. Generic across any future analysis: it takes the dump callback as a parameter instead of knowing `PipelineState`'s shape.
  - `crawl.analyses.tour.render` module exposing every function `workflow/__main__.py` had except `main()`: `md_to_html`, `mermaid_label`, `build_mermaid`, `staleness_disclaimer`, `chapter_html_name`, `available_lenses`, `write_text`, `build_related_links`, `write_chapter_files`, `write_index_md`, `write_index_html`, `format_session_summary`, `estimate_dry_run_cost`, `format_dry_run_summary`, `dump_run_state`, `default_output_dir`, plus the module-level constants `MERMAID_LEGEND`, `SHARED_STYLE`, `MERMAID_SCRIPT`, `INDEX_HTML_TEMPLATE`, `CHAPTER_HTML_TEMPLATE`, `DRY_RUN_CHAPTER_GUESS`.
  - `crawl.analyses.tour`: `NAME = "tour"`, `build_flow() -> Flow`, `add_arguments(parser) -> None`, `init_shared(args) -> dict`, `run(args) -> None` — consumed by `cli.py` and by `crawl.analyses.ANALYSES`. (The spec's sketch showed `init_shared(args, out_dir)`; tour's own pipeline never reads `out_dir` from `shared` — every render function that needs it takes `out` as an explicit argument — so the unused parameter is dropped here rather than carried as speculative scaffolding. Add it back if a future analysis genuinely needs `out_dir` inside `shared`.)
  - `crawl.analyses.ANALYSES = {"tour": crawl.analyses.tour}` — consumed by `cli.py`.
  - `crawl.cli.main() -> None` — the new console-script entry point.

- [ ] **Step 1: Create `src/crawl/core/runner.py`**
...
- [ ] **Step 2: Create `src/crawl/analyses/tour/render.py`**
...
`get_usage` is not imported here: no function in `render.py` calls it. It's called by `tour/__init__.py`'s `run()` (Step 4 below), which imports it directly from `crawl.core`. `read_prompt` stays because `estimate_dry_run_cost` uses it.
...
Run: `python -c "from crawl.analyses.tour import render; print(render.MERMAID_LEGEND)"`
Expected: prints the legend string with no import errors

- [ ] **Step 4: Create `src/crawl/analyses/tour/__init__.py`**
...
- [ ] **Step 5: Create `src/crawl/analyses/__init__.py`**
...
from crawl.analyses import tour

ANALYSES = {tour.NAME: tour}
```

- [ ] **Step 6: Create `src/crawl/cli.py`**
...
(The `[project]` table already exists in `pyproject.toml` from Task 1 with `name = "coderay"` — change just that one line to `"crawl"` here, don't duplicate the table. This is the first point where the rename is safe: `workflow/__main__.py` and its `version('coderay')` call are deleted in this same task, in Step 7 below, so nothing left in the tree still depends on the old distribution name.)

**Keep `package-dir = {"" = "src"}`.** setuptools' src-layout auto-discovery only activates when `packages` is _not_ set explicitly. This plan lists `packages` explicitly (so `pyproject.toml` stays an accurate map of what actually ships, matching the file's style since Task 1), which means the `"" = "src"` root mapping must stay too — drop it and setuptools looks for `crawl/` at the repo root and the build fails. What actually goes away here is the _dual_ mapping from Task 1-4 (`"workflow" = "workflow"`, `"coderay_utils" = "coderay_utils"`), since those packages no longer exist.

- [ ] **Step 10: Run the full suite**

Run: `uv lock && uv sync && python -m pytest tests/ -q`
Expected: all 141 tests pass. The old `coderay` console script is gone; only `crawl` exists now.

- [ ] **Step 11: Manual smoke check of the CLI wiring** (no API key needed — `--version` doesn't touch the pipeline)

Run: `python -m crawl.cli --version`
Expected: prints `crawl 0.1.0`

Run: `python -m crawl.cli --help`
...
- [`src/crawl/analyses/tour/`](src/crawl/analyses/tour/) — the `tour` analysis: SmartCrawl, ExtractGraph, Analyze, Relate, WriteChapters. SmartCrawl, Analyze, and Relate parse structured YAML output and retry on a bad response (`crawl.core.yaml_call`); WriteChapters calls the LLM directly and retries via PocketFlow's own `Node(max_retries=3)`.
...
Line 21: `python -m workflow path/to/repo   # or just: coderay path/to/repo` → `crawl tour path/to/repo`

Lines 58-60, 73 (example invocations): `python -m workflow path/to/repo --instructions X` → `crawl tour path/to/repo --instructions X`; `python -m workflow path/to/repo --dry-run` → `crawl tour path/to/repo --dry-run`

Line 55: `Same pipeline and code, driven by a different file under \`workflow/instructions/\``→ `` driven by a different file under`src/crawl/analyses/tour/instructions/` ``

Line 96: `Built-in pricing covers the default model for each provider. For any other model, coderay prompts you once, interactively, for $/1M token pricing, and saves it to \`~~/.config/coderay/pricing.json\`.`→ `` ...crawl prompts you once... saves it to`~~/.config/crawl/pricing.json`. ``

Line 118: ``the selection rules in [`workflow/prompts/select-files.md`](workflow/prompts/select-files.md)`` → ``the selection rules in [`src/crawl/analyses/tour/prompts/select-files.md`](src/crawl/analyses/tour/prompts/select-files.md)``

- [ ] **Step 2: Update `CONTRIBUTING.md`**

Line 10: ``a key only to run the pipeline against a real repo (`python -m workflow path/to/repo`)`` → ``(`crawl tour path/to/repo`)``

Line 24: ``Parse LLM YAML output through `coderay_utils.yaml_call`... Do not add a new parse-and-retry loop in `workflow/nodes.py`.`` → ``through `crawl.core.yaml_call`... in `src/crawl/analyses/tour/nodes.py`.``

Line 25: `` `workflow/__main__.py` once had a confirmed stored-XSS ... before you change that file's rendering code. `` → `` `src/crawl/analyses/tour/render.py` once had a confirmed stored-XSS (cross-site scripting) bug here. Read `.full-review/02a-security.md` before you change that file's rendering code. ``

Line 27: ``Add a new output lens by adding one file to `workflow/instructions/`.`` → ``to `src/crawl/analyses/tour/instructions/`.``

- [ ] **Step 3: Update `CLAUDE.md`**

Line 68: `pip install -e .              # install this package (workflow/ + coderay_utils/) in editable mode` → `# install this package (src/crawl/) in editable mode`

Line 72: `python -m workflow path/to/repo   # run the pipeline end to end (needs an API key, see .env.example)` → `crawl tour path/to/repo   # run the tour analysis end to end (needs an API key, see .env.example)`
...
Line 87: `` `workflow/__main__.py` renders the pipeline's output (`shared` dict, typed as `PipelineState` in `workflow/nodes.py`) to markdown + HTML. `` → `` `src/crawl/analyses/tour/render.py` renders the analysis's output (`shared` dict, typed as `PipelineState` in `src/crawl/analyses/tour/nodes.py`) to markdown + HTML. ``

Line 88: `` `workflow/prompts/*.md` are the four LLM prompt templates; `workflow/instructions/*.md` are swappable output lenses... `` → `` `src/crawl/analyses/tour/prompts/*.md`... `src/crawl/analyses/tour/instructions/*.md`... ``

Line 94: ``see `.full-review/02a-security.md` before touching `workflow/__main__.py`'s rendering code.`` → ``before touching `src/crawl/analyses/tour/render.py`'s rendering code.``

Line 95: ``a bespoke `parse_yaml`/`.format()` combo in `workflow/nodes.py`... Reuse `coderay_utils.yaml_call` and `coderay_utils.fill()`...`` → ``in `src/crawl/analyses/tour/nodes.py`... Reuse `crawl.core.yaml_call` and `crawl.core.fill()`...``
...
One of `beginner-tutorial`, `architecture-review`, `security-audit`, `onboarding-guide` — a file in `src/crawl/analyses/tour/instructions/`, selected with `--instructions`.
```
...
git commit -m "Update docs and Makefile for the crawl CLI and src/crawl/ layout"
```
...
- Consumes: `crawl.cli.main` (Task 5), a fixture repo under `tests/fixtures/` (created in this task if one doesn't already exist).
...
git commit -m "Add end-to-end smoke test for crawl tour, skipped without an API key"
```
...
1. Open a PR from `worktree-coderay-crawl-restructure` against `main`.
