...
Copy `.claude/commands/epic-loop.md`. The parts specific to this repo are the test command (`uv run pytest tests/ -q`) and the `.beads/interactions.jsonl` stash around branch switches. The review commands, the fix-or-file rule and the decision-bead rule carry over unchanged.
