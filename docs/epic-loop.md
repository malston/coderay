...
Claude Code can work every open child of a beads epic without you driving each step: one branch and one PR per bead, a review pass or two depending on the bead's priority, the findings fixed or filed, the PR merged, the bead closed, then the next one. This page is how to start it, what it does on each pass, and how to stop it.
...
- **You are fine with Claude merging.** Every PR gets a `/code-review` pass; a PR carrying a bead of priority P0 to P3 gets the `/pr-review-toolkit:review-pr` agents as well, and a PR whose beads are all P4 does not. The Critical and Important findings, and any correctness or security defect, are fixed before the merge, but no human reads the diff first. If you want to read them, remove the `gh pr merge` line from step 7 of the command file and merge by hand; the cleanup and the bead close stay.
...
5. Runs `/code-review <PR#> medium` against the PR's own diff, plus the applicable `review-pr` agents when the PR carries a bead of priority P0 to P3. A PR whose beads are all P4 stops at `/code-review`.
