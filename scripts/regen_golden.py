...
Run this with a plain interpreter. The sibling's package is imported from its
`src/`, and the script refuses to write if that import resolves anywhere else.
"""
import argparse
import json
import os
...
# Places where crawl deliberately differs from the port source. Every entry is
# applied to the generated HTML, so the fixture keeps deriving from the pinned
# upstream commit rather than from crawl's own renderer, which would make the
# golden test check crawl against itself. Each entry must still match, so a
...
# product-intent only mermaid. crawl.core.render defers to their render_html,
...
def main():
    ...
    if not resolved.startswith(sibling_root + os.sep):
        sys.exit(f"imported crack from {resolved}, not under {sibling_root}.\n"
                 ...
                 f"installed copy. Refusing to write golden files generated by "
                 f"the wrong package. Run this script with an interpreter whose "
                 f"path holds no other copy of the sibling's package.")
    print(f"using the sibling's crack from {resolved}")
