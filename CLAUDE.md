...
- Every analysis declares `preview(args)` beside `add_arguments`/`init_shared`/`sent`/`run`: it runs only that analysis's crawl step -- pure filesystem work, no LLM call -- and returns `{"counts": {label: int}, "files": {label: [rel]}, "notes": [str]}`. `crawl estimate-token-usage <analysis> <repo_path>` prints it. The labels are deliberately per-analysis: only `product-intent` tracks included/dropped/unreadable, and reporting a count a crawler never computed would be inventing a number. Where a budget caps text instead of dropping files (`architecture` truncates its bundle, `schema` caps per-file text) the counts alone overstate what the model reads, so that goes in `notes`; so does a shallow clone, whose commit count is a fragment (coderay-q2r.38). `tour`'s file preview is sized by `preview_budget`, not `--codebase-budget`, which reaches only its later prompts.
