...
- The voice every report is written in lives in one shipped block, `src/crack/core/prompts/house-style.md`, filled into a prompt's `{house_style}` slot by `crack.core.read_prompt` (coderay-aph). Read that file before writing or editing a prompt template, an instructions lens, or renderer copy. A prose prompt carries the slot in its static prefix and does not restate the rules; a prompt whose reply is parsed, or whose reader is deliberately not an engineer, has no slot. `tests/test_prompts.py` pins which is which.
