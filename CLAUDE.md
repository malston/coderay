...
- The only card-family analysis with a schema-specific flag of its own (`--schema`; every analysis also takes `--codebase-budget`, and tour has `--instructions`), the only one that retitles the page from LLM output (`THEME.page_name`), and the only one using `when_empty="skip-note"`. TableDeepDive batches four tables per call, which is why its `ENV_DEFAULTS` is empty where the other card analyses raise `LLM_MAX_OUTPUT_TOKENS`.
