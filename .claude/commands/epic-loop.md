...
5. Run `/code-review <PR#> medium` and `/pr-review-toolkit:review-pr <PR#>`. Tell every review agent to read `gh pr diff <n>` and `git show origin/<branch>:<path>`, never the working tree. Stay in the bead's worktree until its fixes are pushed.
