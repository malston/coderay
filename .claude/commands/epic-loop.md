...
5. Run `/code-review <PR#> medium`. For a bead of priority P3 or higher, also run `/pr-review-toolkit:review-pr <PR#>`; a P4 gets the standalone review alone. Tell every review agent to read `gh pr diff <n>` and `git show origin/<branch>:<path>`, never the working tree. Stay in the bead's worktree until its fixes are pushed.
