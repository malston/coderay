from crawl.core.pricing import cost_for, get_price
...
def test_override_file_wins_over_builtin_table():
    from crawl.core.pricing import _save_override, get_price
...
def test_get_price_treats_non_dict_top_level_override_file_as_no_overrides(tmp_path):
    import crawl.core.pricing as pricing_module
    from crawl.core.pricing import get_price
...
def test_load_overrides_warns_on_corrupt_file_and_returns_empty(capsys):
    import crawl.core.pricing as pricing_module
    from crawl.core.pricing import _load_overrides
...
def test_prompt_skips_on_non_tty_stdin(monkeypatch):
    from crawl.core.pricing import prompt_for_pricing
...
def test_prompt_writes_entered_values_to_override_file(monkeypatch):
    from crawl.core.pricing import get_price, prompt_for_pricing
...
def test_ensure_priced_does_not_prompt_for_a_known_model(monkeypatch):
    from crawl.core.pricing import ensure_priced
...
def test_ensure_priced_prompts_for_an_unknown_model_on_a_tty(monkeypatch):
    from crawl.core.pricing import ensure_priced, get_price
...
def test_input_ceiling_is_none_for_a_model_with_no_recorded_figure():
    from crawl.core.pricing import input_ceiling

    assert input_ceiling("openai", "gpt-6-nonexistent") is None


def test_the_recorded_ceilings_match_the_published_figures():
    """Each is the input figure from the vendor's own page, except Anthropic's,
    which comes from the API's refusal text. Whole token counts, not
    per-million fractions: a ceiling routed through the $/token conversion the
    price table applies would come back as 1.0.

    The OpenAI figure is the one most easily got wrong. Its page publishes an
    input limit, a context window and an output cap, and the window is the
    input limit plus the output cap, so the wrong row looks plausible and
    leaves the guard permissive by 128,000 tokens."""
    from crawl.core.pricing import input_ceiling

    assert input_ceiling("anthropic", "claude-sonnet-5") == 1_000_000
    assert input_ceiling("openai", "gpt-5.6-terra") == 922_000
    assert input_ceiling("gemini", "gemini-3.7-flash") == 1_048_576


def test_the_openai_ceiling_is_the_input_row_not_the_window():
    """Pins the distinction directly, so transposing the rows fails with a
    message naming the mistake rather than an opaque value mismatch."""
    from crawl.core.pricing import input_ceiling

    recorded = input_ceiling("openai", "gpt-5.6-terra")
    assert recorded != 1_050_000, (
        "1,050,000 is gpt-5.6-terra's context window, not its input limit; "
        "its page publishes 'Maximum input tokens: 922,000'")
    assert recorded == 922_000
